...
function get_coin_list(api_key, cb) {
  request({ uri: base_url + 'coins/list?include_platform=false' + (api_key == null || api_key == '' ? '' : '&x_cg_demo_api_key=' + api_key), json: true, headers: { 'User-Agent': 'eIquidus' }}, function (error, response, body) {
    if (error)
      return cb(error, []);
...
function get_simple_price(id, currency, market_array, api_key, cb) {
  request({ uri: base_url + `simple/price?ids=${id.toLowerCase()}&vs_currencies=usd${currency == null || currency == '' ? '' : `,${currency}`}` + (api_key == null || api_key == '' ? '' : '&x_cg_demo_api_key=' + api_key), json: true, headers: { 'User-Agent': 'eIquidus' }}, function (error, response, body) {
    if (error)
      return cb(error, 0, 0);
    else if (body == null || body == '' || typeof body !== 'object')
