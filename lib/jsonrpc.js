  ...
  request.on('response', function(response) {
    ...
    response.on('end', function() {
      ...
      decoded.forEach(function(decodedResponse, i) {
        console.log('DEBUG: decodedResponse', JSON.stringify(decodedResponse));
        if (decodedResponse.hasOwnProperty('error') && decodedResponse.error != null) {
          ...
          if (errback) {
            ...
            if (decodedResponse.error && decodedResponse.error.message) {
              errorMsg = decodedResponse.error.message;
            } else if (decodedResponse.error) {
              errorMsg = JSON.stringify(decodedResponse.error);
            } else {
              errorMsg = 'Unknown RPC error: ' + JSON.stringify(decodedResponse);
            }
